...
pub enum SendResponse {
    ...
    ToOne(Token, Message),
    // send response to a list of client
...
impl Connection {
    ...
    fn send(&self, s: SendResponse) {
        match s {
            ...
            SendResponse::ToOne(token, message) => self.send_to_one(token, message),
    ...
    fn send_to_self(&self, message: Message) {
        ...
        send_message(ct, &self.client.sender, message);
    }
    ...
    fn send_to_one(&self, token: Token, message: Message) {
        ...
        for (t, client) in self.registry.borrow().iter() {
            if t == &token {
                ...
                send_message(ct, client, message.clone());
                break;
            }
        }
    }
...
fn send_message(sender: Token, client: &Sender, message: Message) {
    match client.send(message.clone()) {
        Ok(_) => (),
        Err(err) => {
            error!(target: "connection_events", "client {:?} send message to {:?}: error => {}", sender, client.token(), err);
        }
    }
}
...
impl Handler for Connection {
    ...
    fn on_close(&mut self, _: CloseCode, _: &str) {
        ...
        if self.client.ready {
            ...
            self.send(SendResponse::ToOther(build_message(out)))
        }
    }
    ...
    fn on_message(&mut self, msg: Message) -> Result<()> {
        ...
        match serde_json::from_slice::<WsMessageIn>(&msg.into_data()) {
            ...
            Err(err) => {
                ...
                self.send(SendResponse::ToSelf(build_message(out)))
            }
        }
        Ok(())
    }
